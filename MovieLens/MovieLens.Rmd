...
header-includes:
  # --- 1. Load the packages. We only need fvextra and a configured titlesec ---
  - \usepackage{fvextra}
  
  # Load titlesec with the 'nobottomtitles' option. 
  # This option does exactly what we were trying to do with needspace/etoolbox, but safely.
  - \usepackage[nobottomtitles]{titlesec}

  # --- 2. Configure the packages ---
  # Configure word wrapping for code blocks
  - \DefineVerbatimEnvironment{Highlighting}{Verbatim}{breaklines,commandchars=\\\{\}}
  
  # Configure spacing around titles using the titlesec commands
...
# Check if the 'systemfonts' package is installed; if not, install it from CRAN
if(!require(systemfonts)) 
    install.packages("systemfonts", repos = "http://cran.us.r-project.org")

# Check if the 'kableExtra' package is installed; if not, install it from CRAN
...
# Load systemfonts to prevent font from conflicting
library(systemfonts)

# Load kableExtra to produce well-formatted tables for exploratory analysis
library(kableExtra)
```
...
1.  Data Wrangling and Cleaning To begin with, key variables such as
    userId, movieId, rating, timestamp, and genres must be thoroughly
    parsed to ensure data integrity. The dataset should be examined for
    duplicates, missing values, and potentially corrupt entries.
    Anomalous ratings—those falling outside the valid range—should be
    identified and addressed. Using regular expressions, the movie title
    field can be parsed to extract the release year, while timestamps
    should be converted into standard date-time objects to facilitate
    the creation of time-based features (e.g., year, month, day).
    Furthermore, genre information ought to be transformed into a format
    suitable for modeling, such as multi-hot encoding or a normalized
    long-table format.

2.  Auditing for Bias A comprehensive bias audit involves evaluating the
    distribution of rating counts at both the user and movie levels to
    assess potential participation imbalances. Comparative analyses
    should be conducted across genres, time periods, and popularity
    strata to identify disparities in coverage. The relationship between
    the number of ratings and average rating per item can reveal the
    presence of popularity bias. Additionally, one should assess
    representation gaps, focusing on users or movies with limited
    interaction history, such as obscure genres or newly introduced
    items. Differences in rating behavior across user frequency
    tiers—high-frequency versus low-frequency users—should also be
    analyzed to expose behavioral heterogeneity.

3.  Exploratory Trend Analysis This phase entails a visual exploration
    of global rating distributions, with stratification by genre,
    temporal dimension, and user activity level. Temporal trends in
    rating behaviors can be captured via line plots segmented by rating
    year, or heatmaps mapped by genre over time. Analysis of user and
    movie activity trajectories can uncover patterns such as user churn
    or content consumption surges. To probe cultural or temporal shifts,
    average ratings can be compared across genres and release decades.
    Scatter plots serve to illustrate relationships between rating and
    movie age, as well as to explore the interaction between rating
    variance and sample size.

4.  Feature Engineering Preparation For effective predictive modeling,
    several derived features should be constructed. Time-sensitive
    features such as movie_age, rating_year, and user_tenure enrich the
    temporal resolution of the dataset. Normalization techniques should
    be employed to adjust ratings by user or movie means to reduce
    idiosyncratic bias. Genre information may be encoded through one-hot
    vectors, multi-hot matrices, or embedded representations suitable
    for downstream algorithms. Users and movies may be binned according
    to their activity levels, enabling the generation of aggregate
    statistics per bin. Finally, statistical outliers—users with
    anomalous rating variance or consistent directional bias—should be
    flagged to inform data preprocessing or modeling strategies.
...
grViz("
digraph academic_pipeline {
  graph [layout=dot, rankdir=TB, bgcolor='white', compound=true, 
         nodesep=0.6, ranksep=1.0, ratio='auto']
  node [shape=rectangle, style='filled,rounded', fontname='Arial', margin=0.2, width=2, fontsize=20]
  edge [arrowsize=0.8, fontsize=16]

  # 1. DATA WRANGLING & AUDITING
  subgraph cluster_data {
    label=<<TABLE BORDER='0'><TR><TD><FONT POINT-SIZE='22'><B>1. DATA WRANGLING &amp; AUDITING</B></FONT></TD></TR></TABLE>>
    style='filled,rounded'
    fillcolor='#B3E5FC'

    wrangling [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Wrangling &amp; Cleaning</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Parse all key variables</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Handle duplicates, missing, outliers</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Parse release year; convert date-times</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Split genres into indicators</FONT></TD></TR>
    </TABLE>>, fillcolor='#E1F5FE']

    audit [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Audit for Bias</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Rating distributions</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Genre, user/movie coverage</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Pop/underrepresented groups</FONT></TD></TR>
    </TABLE>>, fillcolor='#E1F5FE']

    trends [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Exploratory Trends</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Global/genre/year rating trends</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• User activity, temporal/cultural</FONT></TD></TR>
    </TABLE>>, fillcolor='#E1F5FE']

    feature_prep [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Feature Prep</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Time-based features</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Normalize ratings &amp; encode genres</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Bin/group users/movies, outlier detect</FONT></TD></TR>
    </TABLE>>, fillcolor='#E1F5FE']
  }

  # 2. MODELING
  subgraph cluster_modeling {
    label=<<TABLE BORDER='0'><TR><TD><FONT POINT-SIZE='22'><B>2. MODEL DEVELOPMENT &amp; EVALUATION</B></FONT></TD></TR></TABLE>>
    style='filled,rounded'
    fillcolor='#D1C4E9'

    model_select [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Model Selection/spec</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Identify &amp; justify models</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• List features &amp; assumptions</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    prep_fe [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Data Prep &amp; FE</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Preprocess for each model</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Standardize FE steps, handle imbalance</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    training [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Training &amp; Tuning</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Train on same data, cross-validate</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Log performance</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    reg [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Regularization</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Try penalty/complexity terms</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Compare effects</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    eval [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Evaluation</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• RMSE, MAE, ranking if needed</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Performance table</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    ensemble [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Ensemble (Opt)</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Stack, blend</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Record gains/trade-offs</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']

    report [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Reporting/Repro</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Document choices, include stats/visuals</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Ensure code reproducibility</FONT></TD></TR>
    </TABLE>>, fillcolor='#EDE7F6']
  }

  # 3. (OPTIONAL) - Spacer cluster for visual separation
  subgraph cluster_spacer {
    style='invis'
    invisible1 [label='', shape=point, width=0.01, height=0.1, style='invis']
    invisible2 [label='', shape=point, width=0.01, height=0.1, style='invis']
    invisible3 [label='', shape=point, width=0.01, height=0.1, style='invis']
    invisible4 [label='', shape=point, width=0.01, height=0.1, style='invis']
    invisible5 [label='', shape=point, width=0.01, height=0.1, style='invis']
  }

  # 4. ACADEMIC WRAP-UP (YELLOW COLUMN)
  subgraph cluster_academic {
    label=<<TABLE BORDER='0'><TR><TD><FONT POINT-SIZE='22'><B>4. ACADEMIC WRAP-UP</B></FONT></TD></TR></TABLE>>
    style='filled,rounded'
    fillcolor='#FFF9C4'

    summary [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Summary &amp; Motivation</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Recap objectives/data/method</FONT></TD></TR>
    </TABLE>>, fillcolor='#FFFDE7']

    results [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Results &amp; Interpretation</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Main findings, metrics, visuals</FONT></TD></TR>
    </TABLE>>, fillcolor='#FFFDE7']

    limitations [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Limitations/Bias</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Data/model limits</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Recommendations</FONT></TD></TR>
    </TABLE>>, fillcolor='#FFFDE7']

    reproducibility [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Reproducibility Appx</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Code repo, dependencies</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Data/model artifacts</FONT></TD></TR>
    </TABLE>>, fillcolor='#FFFDE7']

    conclusion [label=<<TABLE BORDER='0' CELLPADDING='4'>
      <TR><TD><FONT POINT-SIZE='18'><B>Conclusion &amp; Next Steps</B></FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Synthesize findings</FONT></TD></TR>
      <TR><TD><FONT POINT-SIZE='16'>• Suggest future/applied work</FONT></TD></TR>
    </TABLE>>, fillcolor='#FFFDE7']
  }
  ...
  # Main Flows
  wrangling -> audit -> trends -> feature_prep
  feature_prep -> model_select [label='Ready for modeling', style=dashed, color='#555']
  model_select -> prep_fe -> training -> reg -> eval -> ensemble -> report
  report -> summary [label='To write-up', style=dashed, color='darkgreen']
  summary -> results -> limitations -> reproducibility -> conclusion

  # Vertical alignment (columns stay lined up by same rank)
  { rank=same; wrangling; model_select; invisible1; summary; }
  { rank=same; audit; prep_fe; invisible2; results; }
  { rank=same; trends; training; invisible3; limitations; }
  { rank=same; feature_prep; reg; invisible4; reproducibility; }
  { rank=same; eval; ensemble; invisible5; conclusion; }
}
")
```