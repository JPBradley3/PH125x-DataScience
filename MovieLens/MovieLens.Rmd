...
```{r workflow, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7.5, fig.height=10, dpi=300, fig.cap="Project Workflow"}
library(DiagrammeR)

# NOTE: The only change is in the R chunk header above.
# We replaced `out.width` and `fig.asp` with explicit `fig.width` and `fig.height` values in inches.
# This provides the absolute dimensions needed for the PDF rendering engine.
...
digraph pipeline {
  ...
  graph [
    ...
    compound=true,
    pad=0.5
    # The 'size' attribute remains REMOVED.
  ]
