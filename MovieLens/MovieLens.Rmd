...
edx_sample <- edx_sample %>%
  mutate(
    ...
    year = lubridate::year(lubridate::as_datetime(timestamp))
  )
...
# Plot 1: Smoothed Average Rating by Month
...
# Fit LOESS model for smooth trend
...
# Calculate local slope (first derivative)
d_pred <- diff(preds) / as.numeric(diff(fine_dates))
mid_dates <- fine_dates[-1] # Center points for derivative
...
# Plot 2: Trend with Slope Coloring (LaTeX-friendly label)
ggplot() +
  ...
  scale_color_gradient2(
    ...
    name = expression(paste("Slope (", Delta, " rating/day)"))
  ) +
  theme_light(base_size = 15) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(
